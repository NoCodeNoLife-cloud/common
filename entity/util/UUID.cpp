...
{
	UUID::UUID() = default;
	UUID::UUID(const uint64_t mostSigBits, const uint64_t leastSigBits): mostSignificantBits(mostSigBits), leastSignificantBits(leastSigBits) {}
	UUID::~UUID() = default;
	/// \brief Generates a random UUID.
	/// \return a random UUID.
	auto UUID::randomUUID() -> UUID
	{
	...
	auto UUID::fromString(const std::string& name) -> UUID
	{
		uint64_t msb = 0;
		uint64_t lsb = 0;
		size_t idx = 0;
	...
	auto UUID::getMostSignificantBits() const -> uint64_t
	{
	...
	auto UUID::getLeastSignificantBits() const -> uint64_t
	{
	...
	auto UUID::toString() const -> std::string
	{
		...
		oss << std::hex << std::setfill('0')
			<< std::setw(8) << (mostSignificantBits >> 32) << '-'
			<< std::setw(4) << ((mostSignificantBits >> 16) & 0xFFFF) << '-'
			<< std::setw(4) << (mostSignificantBits & 0xFFFF) << '-'
			<< std::setw(4) << (leastSignificantBits >> 48) << '-'
			<< std::setw(12) << (leastSignificantBits & 0xFFFFFFFFFFFFULL);
		return oss.str();
	}
	...
	auto UUID::equals(const UUID& other) const -> bool
	{
	...
	/// \param other The UUID to compare with.
	/// \return A negative integer, zero, or a positive integer as the UUID is less than, equal to, or greater than the other UUID.
	auto UUID::compareTo(const UUID& other) const -> int
	{
	...
	auto UUID::hashCode() const -> int
	{
		return static_cast<int>((mostSignificantBits >> 32) ^ mostSignificantBits ^
	...
	auto UUID::nameUUIDFromBytes(const std::vector<uint8_t>& name) -> UUID
	{
	...
	auto UUID::generateRandom64Bits() -> uint64_t
	{
