...
UUID::UUID(const uint64_t mostSigBits, const uint64_t leastSigBits) : mostSignificantBits(mostSigBits),
...
auto UUID::randomUUID() -> UUID { return {generateRandom64Bits(), generateRandom64Bits()}; }
...
auto UUID::getMostSignificantBits() const -> uint64_t { return mostSignificantBits; }

/// \brief Returns the least significant bits of the UUID.
/// \return The least significant 64 bits of the UUID.
auto UUID::getLeastSignificantBits() const -> uint64_t { return leastSignificantBits; }
...
auto UUID::equals(const UUID& other) const -> bool { return mostSignificantBits == other.mostSignificantBits && leastSignificantBits == other.leastSignificantBits; }
...
auto UUID::hashCode() const -> int { return static_cast<int>(mostSignificantBits >> 32 ^ mostSignificantBits ^ leastSignificantBits >> 32 ^ leastSignificantBits); }
...
auto UUID::nameUUIDFromBytes(const std::vector<unsigned char>& name) -> UUID {
