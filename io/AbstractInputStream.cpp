...
auto AbstractInputStream::read(std::vector<char>& buffer) -> size_t {
...
/// \return The total number of bytes read into the buffer, or -1 if the end of the stream has been reached.
auto AbstractInputStream::read(std::vector<char>& buffer, const size_t offset, const size_t len) -> size_t {
