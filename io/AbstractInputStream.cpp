...
auto AbstractInputStream::read(std::vector<char>& buffer) -> int {
...
/// \return The total number of bytes read into the buffer, or -1 if the end of the stream has been reached.
auto AbstractInputStream::read(std::vector<char>& buffer, const int offset, const int len) -> int {
...
auto AbstractInputStream::skip(const int64_t n) -> int64_t {
	if (n < 0) {
		throw std::invalid_argument("Skip value cannot be negative");
	}
	int64_t skipped = 0;
