...
namespace common::io
{
BufferedInputStream::BufferedInputStream(AbstractInputStream& in): BufferedInputStream(in, DEFAULT_BUFFER_SIZE) {}

BufferedInputStream::BufferedInputStream(AbstractInputStream& in, const int size): buf_(size),
                                                                           inputStream_(in) {
	if (!&inputStream_) {
...
auto BufferedInputStream::markSupported() -> bool {
	return true;
}
...
auto BufferedInputStream::read(std::vector<char>& buffer, size_t offset, size_t len) -> size_t {
