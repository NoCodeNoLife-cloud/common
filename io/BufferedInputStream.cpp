...
namespace common::io
{
BufferedInputStream::BufferedInputStream(InputStream& in): BufferedInputStream(in, 8192) {}

BufferedInputStream::BufferedInputStream(InputStream& in, const int size): buf_(size),
                                                                           inputStream_(in) {
	if (!&inputStream_) { throw std::invalid_argument("Input stream cannot be null"); }
...
auto BufferedInputStream::available() const -> size_t { return count_ - pos_ + inputStream_.available(); }
...
auto BufferedInputStream::markSupported() -> bool { return true; }
...
auto BufferedInputStream::read() -> int {
	if (pos_ >= count_) {
		fillBuffer();
...
/// \details This function reads a portion of the stream into the provided byte array.
/// The buffer is filled with bytes starting at the current position in the stream, up to
/// a maximum of `len` bytes. If the end of the stream is reached before filling the buffer,
/// it is filled as much as possible and the number of bytes read is returned.
/// \param buffer The byte array to fill.
/// \param offset The starting offset in the byte array.
/// \param len The maximum number of bytes to read.
/// \return The total number of bytes read into the buffer, or -1 if the end of the stream is reached.
auto BufferedInputStream::read(std::vector<unsigned char>& buffer, size_t offset, size_t len) -> size_t {
	if (offset + len > static_cast<int>(buffer.size())) { throw std::out_of_range("Buffer offset/length out of range"); }
...
auto BufferedInputStream::skip(long long n) -> long long {
	if (n <= 0) { return 0; }
	...
	while (n > 0) {
		...
		const auto bytesToSkip = std::min(static_cast<long long>(bytesAvailable), n);
		pos_ += bytesToSkip;
		n -= bytesToSkip;
		skipped += bytesToSkip;
	}
...
auto BufferedInputStream::fillBuffer() -> void {
	if (markPos_ < 0 || pos_ - markPos_ >= markLimit_) { markPos_ = -1; }

	if (const int bytesRead = inputStream_.read(buf_); bytesRead > 0) {
		pos_ = 0;
		count_ = bytesRead;
	}
	else { count_ = 0; }
