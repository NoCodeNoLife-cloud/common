...
namespace common::io
{
BufferedOutputStream::BufferedOutputStream(AbstractOutputStream& out): BufferedOutputStream(out, DEFAULT_BUFFER_SIZE) {}

BufferedOutputStream::BufferedOutputStream(AbstractOutputStream& out, const size_t size): outputStream_(out),
                                                                                  bufferSize_(size),
                                                                                  buffer_(size),
                                                                                  bufferPosition_(0) {
	if (!&outputStream_) {
...
auto BufferedOutputStream::write(const int b) -> void {
	...
	buffer_[bufferPosition_++] = static_cast<char>(b);
}
...
auto BufferedOutputStream::write(const std::vector<char>& data, const size_t offset, const size_t len) -> void {
