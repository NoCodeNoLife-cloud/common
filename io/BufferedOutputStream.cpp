...
namespace common::io
{
BufferedOutputStream::BufferedOutputStream(OutputStream& out): BufferedOutputStream(out, DEFAULT_BUFFER_SIZE) {}

BufferedOutputStream::BufferedOutputStream(OutputStream& out, const size_t size): outputStream_(out),
                                                                                  bufferSize_(size),
                                                                                  buffer_(size),
                                                                                  bufferPosition_(0) {
	if (!&outputStream_) { throw std::invalid_argument("Output stream cannot be null"); }
...
auto BufferedOutputStream::write(const int b) -> void {
	if (bufferPosition_ >= bufferSize_) { flushBuffer(); }
	buffer_[bufferPosition_++] = static_cast<unsigned char>(b);
}
...
auto BufferedOutputStream::write(const std::vector<unsigned char>& data, const size_t offset, const size_t len) -> void {
...
auto BufferedOutputStream::close() -> void { outputStream_.close(); }
