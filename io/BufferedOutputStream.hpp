...
class BufferedOutputStream final : public interface::IfaceFlushable,
                                   public interface::IfaceCloseable
{
public:
	explicit BufferedOutputStream(AbstractOutputStream& out);
	BufferedOutputStream(AbstractOutputStream& out, size_t size);
	~BufferedOutputStream() override;
	auto write(int b) -> void;
	auto write(const std::vector<char>& data, size_t offset, size_t len) -> void;
	auto flush() -> void override;
	auto close() -> void override;

private:
	static constexpr size_t DEFAULT_BUFFER_SIZE = 8192;
	AbstractOutputStream& outputStream_;
	size_t bufferSize_;
