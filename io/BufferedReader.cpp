...
namespace common::io
{
BufferedReader::BufferedReader(AbstractReader& reader, const int size = DEFAULT_BUFFER_SIZE): reader_(reader),
                                                                                      bufferSize_(size) {
	if (size <= 0) {
...
auto BufferedReader::mark(const int readAheadLimit) -> void {
	if (readAheadLimit <= 0) {
...
auto BufferedReader::markSupported() -> bool {
	return true;
}
...
/// \brief Reads bytes into a specified buffer from the stream.
/// \details This method attempts to read up to len bytes into the provided buffer vector,
/// starting at the specified offset. It fills the buffer with bytes from the internal
/// buffer of the BufferedReader. If the internal buffer is empty, it attempts to fill
/// it from the underlying reader. The method continues to read until the requested
/// number of bytes is read, or the end of the stream is reached. If the offset and
/// length exceed the size of the buffer, an exception is thrown.
