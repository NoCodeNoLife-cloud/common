...
namespace common::io
{
BufferedReader::BufferedReader(Reader& reader, const int size = DEFAULT_BUFFER_SIZE): reader_(reader),
                                                                                      bufferSize_(size) {
...
auto BufferedReader::markSupported() -> bool { return true; }
...
auto BufferedReader::read(char* cBuf, size_t off, size_t len) -> size_t {
	if (off + len > static_cast<int>(buffer_.size())) { throw std::out_of_range("Invalid offset or length"); }

	size_t bytesRead = 0;
	while (len > 0) {
		if (pos_ >= count_) { if (!fillBuffer()) { return bytesRead > 0 ? bytesRead : -1; } }

		const size_t bytesToCopy = std::min(len, count_ - pos_);
		std::memcpy(cBuf + off, buffer_.data() + pos_, bytesToCopy);
		pos_ += bytesToCopy;
		off += bytesToCopy;
		len -= bytesToCopy;
...
auto BufferedReader::ready() const -> bool { return reader_.ready(); }
...
bool BufferedReader::fillBuffer() {
	pos_ = 0;
	count_ = reader_.read(buffer_.data(), 0, bufferSize_);
	return count_ > 0;
}
} // namespace common::io
