...
namespace commom::io
{
BufferedWriter::BufferedWriter(std::ofstream& os, const size_t size = DEFAULT_BUFFER_SIZE): outStream_(os),
                                                                                            bufferSize_(size) {
	if (!os.is_open()) {
...
auto BufferedWriter::write(std::vector<char>& cBuf, const size_t off, const size_t len) -> void {
	if (off + len > cBuf.size()) {
		throw std::out_of_range("Offset and length are out of the bounds of the buffer.");
	}
	if (len > bufferSize_) {
		flush();
		outStream_.write(cBuf.data() + off, static_cast<std::streamsize>(len));
	}
	else {
...
auto BufferedWriter::flush() -> void {
	if (!buffer_.empty()) {
		outStream_.write(buffer_.data(), static_cast<std::streamsize>(buffer_.size()));
		buffer_.clear();
	}
}
