...
namespace common::io
{
ByteArrayInputStream::ByteArrayInputStream(const std::vector<char>& buf): buffer_(buf) {}
...
auto ByteArrayInputStream::read() -> int {
...
auto ByteArrayInputStream::skip(const std::size_t n) -> std::size_t {
	const std::size_t available = buffer_.size() - pos_;
	const std::size_t bytesToSkip = std::min(n, available);
	pos_ += bytesToSkip;
	return bytesToSkip;
}
...
auto ByteArrayInputStream::read(std::vector<char>& cBuf, const std::size_t off, std::size_t len) -> std::size_t {
	if (off + len > cBuf.size()) {
		throw std::out_of_range("Offset and length exceed the size of the buffer");
	}
	if (const std::size_t remaining = buffer_.size() - pos_; len > remaining) {
		len = remaining;
	}
	if (pos_ >= buffer_.size()) {
		return 0;
...
std::size_t ByteArrayInputStream::available() const {
