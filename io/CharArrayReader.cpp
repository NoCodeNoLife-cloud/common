...
namespace common::io
{
CharArrayReader::CharArrayReader(const std::vector<char>& buffer): buf_(buffer),
                                                                   count(buffer.size()) {}
...
auto CharArrayReader::read(std::vector<char>& b, const size_t off, const size_t len) -> size_t {
	...
	const size_t toRead = std::min(len, count - pos);
	std::copy_n(buf_.begin() + static_cast<std::ptrdiff_t>(pos), toRead, b.begin() + static_cast<std::ptrdiff_t>(off));
	pos += toRead;
	return toRead;
}
...
size_t CharArrayReader::skip(const size_t n) {
	const size_t skipped = std::min(n, count - pos);
	pos += skipped;
	return skipped;
}
...
/// \details This method clears the buffer and resets the position, marked position, and count to zero.
