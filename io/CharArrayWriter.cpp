...
auto CharArrayWriter::write(const std::vector<char>& cBuf, const size_t off, const size_t len) -> void {
	...
	if (count + len > static_cast<int>(buf.size())) {
		buf.resize(count + len);
	}
	std::copy_n(cBuf.begin() + static_cast<std::ptrdiff_t>(off), len, buf.begin() + static_cast<std::ptrdiff_t>(count));
	count += len;
}
...
void CharArrayWriter::write(const std::string& str, const size_t off, const size_t len) {
	...
	std::copy_n(str.begin() + static_cast<std::ptrdiff_t>(off), len, buf.begin() + static_cast<std::ptrdiff_t>(count));
	count += len;
}
