...
auto File::compareTo(const File& other) const -> int { return filePath_.string().compare(other.filePath_.string()); }
...
auto File::equals(const File& other) const -> bool { return equivalent(filePath_, other.filePath_); }
...
auto File::createNewFile() const -> bool {
	if (std::filesystem::exists(filePath_)) { return false; }
...
auto File::deleteFile() const -> bool { return std::filesystem::remove(filePath_); }

/// \brief Checks if the file exists.
/// \return true if the file exists, false otherwise.
auto File::exists() const -> bool { return std::filesystem::exists(filePath_); }

/// \brief Returns the absolute path of the file.
/// \return The absolute path of the file.
auto File::getAbsolutePath() const -> std::string { return absolute(filePath_).string(); }
...
auto File::getAbsoluteFile() const -> File { return File(absolute(filePath_).string()); }

/// \brief Returns the name of the file.
/// \return The name of the file.
auto File::getName() const -> std::string { return filePath_.filename().string(); }

/// \brief Returns the parent directory of the file.
/// \return The parent directory of the file.
auto File::getParent() const -> std::string { return filePath_.parent_path().string(); }
...
auto File::getCanonicalFile() const -> File { return File(canonical(filePath_)); }
...
auto File::getParentFile() const -> File { return filePath_.has_parent_path() ? File(filePath_.parent_path()) : File(std::string("")); }
...
auto File::getPath() const -> std::string { return filePath_.string(); }
...
auto File::hashCode() const -> size_t { return std::hash<std::string>()(filePath_.string()); }
...
/// function.
auto File::isAbsolute() const -> bool { return filePath_.is_absolute(); }
...
auto File::mkdir() const -> bool { return create_directory(filePath_); }
...
auto File::isDirectory() const -> bool { return is_directory(filePath_); }

/// \brief Checks if the file is a regular file.
/// \return true if the file is a regular file, false otherwise.
auto File::isFile() const -> bool { return is_regular_file(filePath_); }
...
auto File::length() const -> long long {
	if (std::filesystem::exists(filePath_) && is_regular_file(filePath_)) { return static_cast<long long>(file_size(filePath_)); }
	return 0;
}
...
auto File::list() const -> std::vector<std::string> {
	...
	if (isDirectory()) { for (const auto& entry : std::filesystem::directory_iterator(filePath_)) { entries.push_back(entry.path().filename().string()); } }
	return entries;
}
...
auto File::toURI() const -> std::string { return "file://" + filePath_.string(); }
} // namespace common::io
