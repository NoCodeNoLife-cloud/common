...
auto FileInputStream::read() -> int {
	char byte;
	if (fileStream_.read(&byte, 1)) {
		return byte;
	}
	return -1;
}
...
auto FileInputStream::read(std::vector<char>& buffer) -> size_t {
...
/// \return the total number of bytes read into the buffer, or -1 if there is no more data because the end of the stream has been reached.
auto FileInputStream::read(std::vector<char>& buffer, const size_t offset, const size_t len) -> size_t {
	if (offset + len > buffer.size()) {
		throw std::invalid_argument("Invalid buffer, offset, or length.");
	}
	fileStream_.read(buffer.data() + offset, static_cast<std::streamsize>(len));
