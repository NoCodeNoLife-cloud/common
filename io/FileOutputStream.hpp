...
{
public:
	explicit FileOutputStream(const std::string& name, bool append);
	explicit FileOutputStream(const char* name, bool append);
	explicit FileOutputStream(const std::filesystem::path& file, bool append);
	~FileOutputStream() override;
	void write(int b) override;
	void write(const std::vector<char>& buffer) override;
	void write(const std::vector<char>& buffer, size_t offset, size_t len) override;
	void close() override;
	void flush() override;
