...
auto FilterInputStream::read() -> int {
	if (!inputStream_) {
		throw std::runtime_error("Input stream is not available");
	}
...
size_t FilterInputStream::read(std::vector<char>& buffer) {
	if (!inputStream_) {
		throw std::runtime_error("Input stream is not available");
	}
...
/// \return The number of bytes read.
size_t FilterInputStream::read(std::vector<char>& buffer, size_t offset, size_t len) {
	if (!inputStream_) {
		throw std::runtime_error("Input stream is not available");
	}
