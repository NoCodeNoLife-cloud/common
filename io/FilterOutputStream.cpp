...
namespace common::io
{
FilterOutputStream::FilterOutputStream(std::unique_ptr<AbstractOutputStream> outputStream): outputStream_(std::move(outputStream)) {}
...
auto FilterOutputStream::write(const int b) -> void {
	if (!outputStream_) {
...
/// \throw std::runtime_error If the underlying stream is unavailable.
void FilterOutputStream::write(const std::vector<char>& buffer) {
	if (!outputStream_) {
		throw std::runtime_error("Output stream is not available");
	}
...
void FilterOutputStream::write(const std::vector<char>& buffer, const size_t offset, const size_t len) {
	if (!outputStream_) {
		throw std::runtime_error("Output stream is not available");
	}
