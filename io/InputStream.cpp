...
auto InputStream::mark(int readLimit) -> void { throw std::runtime_error("mark not supported"); }
...
auto InputStream::markSupported() const -> bool { return false; }
...
auto InputStream::read(std::vector<unsigned char>& buffer) -> int { return read(buffer, 0, static_cast<int>(buffer.size())); }
...
/// \return The total number of bytes read into the buffer, or -1 if the end of the stream has been reached.
auto InputStream::read(std::vector<unsigned char>& buffer, const int offset, const int len) -> int {
	if (offset < 0 || len < 0 || offset + len > buffer.size()) { throw std::out_of_range("Buffer offset/length out of range"); }
	int bytesRead = 0;
	for (int i = 0; i < len; ++i) {
		const int byte = read();
		if (byte == -1) { break; }
		buffer[offset + i] = static_cast<unsigned char>(byte);
		++bytesRead;
	}
	return bytesRead;
}
...
auto InputStream::reset() -> void { throw std::runtime_error("reset not supported"); }
...
auto InputStream::skip(const int64_t n) -> int64_t {
	if (n < 0) { throw std::invalid_argument("Skip value cannot be negative"); }
	int64_t skipped = 0;
