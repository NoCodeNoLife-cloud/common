...
namespace common::io
{
InputStreamReader::InputStreamReader(std::unique_ptr<AbstractReader> input) : reader_(std::move(input)) {
	if (!reader_) {
		throw std::invalid_argument("Input stream cannot be null");
	}
}

InputStreamReader::InputStreamReader(std::unique_ptr<AbstractReader> input, const std::string& charsetName) : reader_(std::move(input)) {
	if (!reader_) {
		throw std::invalid_argument("Input stream cannot be null");
	}
