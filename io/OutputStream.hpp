...
class OutputStream abstract : public interface::Closeable,
                              public interface::Flushable
{
public:
	~OutputStream() override = default;
	virtual auto write(int b) -> void = 0;
	virtual auto write(const std::vector<unsigned char>& buffer) -> void;
	virtual auto write(const std::vector<unsigned char>& buffer, size_t offset, size_t len) -> void;
};
} // namespace common::io
