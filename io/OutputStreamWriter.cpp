...
namespace common::io
{
OutputStreamWriter::OutputStreamWriter(std::unique_ptr<AbstractWriter> outputStream, const std::string& charsetName): outputWriter_(std::move(outputStream)),
...
auto OutputStreamWriter::write(const std::vector<char>& cBuf, size_t off, size_t len) -> void {
	if (closed_) {
...
void OutputStreamWriter::write(const std::string& str, size_t off, size_t len) {
	if (off + len > str.size()) {
...
AbstractWriter& OutputStreamWriter::append(char c) {
	write(c);
	return *this;
}
...
/// \return A reference to the current OutputStreamWriter object.
AbstractWriter& OutputStreamWriter::append(const std::string& csq, size_t start, size_t end) {
