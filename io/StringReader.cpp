...
namespace common::io
{
StringReader::StringReader(std::string s) : source_(std::move(s)),
                                            position_(0),
                                            markPosition_(0),
                                            markSet_(false) {}
...
void StringReader::mark(const int readAheadLimit) {
	if (readAheadLimit < 0) { throw std::invalid_argument("readAheadLimit must be non-negative"); }
...
bool StringReader::markSupported() { return true; }
...
int StringReader::read(char cBuf[], const int off, const int len) {
	if (off < 0 || len < 0 || off + len > source_.size()) { throw std::out_of_range("Buffer bounds are invalid"); }

	if (position_ >= source_.size()) { return -1; }

	const int charsToRead = std::min(len, static_cast<int>(source_.size() - position_));
	for (int i = 0; i < charsToRead; ++i) { cBuf[off + i] = source_[position_++]; }
	return charsToRead;
}
...
void StringReader::reset() {
	if (!markSet_) { position_ = 0; }
	else { position_ = markPosition_; }
}
...
long StringReader::skip(const long ns) {
	if (ns < 0) { throw std::invalid_argument("Skip value must be non-negative"); }
