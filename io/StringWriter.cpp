...
auto StringWriter::write(const char* cBuf, const size_t off, const size_t len) -> void {
	if (!cBuf) { throw std::invalid_argument("Null character buffer provided"); }
	buffer_.write(cBuf + off, static_cast<std::streamsize>(len));
}
