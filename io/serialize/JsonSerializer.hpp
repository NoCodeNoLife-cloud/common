...
#include "io/interface/JsonSerializable.hpp"

namespace common::io::serialize
{
	template <typename T>
	concept DerivedFromJsonSerializable = std::is_base_of_v<interface::JsonSerializable, T>;

	/// \brief Class JsonSerializer abstract
	/// \details This class is an abstract class which provides static methods to serialize and deserialize objects to and from JSON files.
	/// It is used by the Student class to save and load student objects to and from JSON files.
	...
	template <DerivedFromJsonSerializable T>
	auto JsonSerializer::saveStudentToJsonFile(const T& entity, const std::string& filename) -> void
	{
	...
	template <DerivedFromJsonSerializable T>
	auto JsonSerializer::loadStudentFromJsonFile(const std::string& filename) -> T
	{
		T entity{};
