...
namespace common::math
{
	...
	/// \details If the argument is not negative, the result is the same as the argument. If the argument is negative, the result is the negation of the argument.
	/// \param[in] a The double value whose absolute value is to be determined.
	/// \return The absolute value of the argument.
	double Math::abs(const double a)
	{
		return std::fabs(a);
	}
	...
	double Math::acos(const double a)
	{
		return std::acos(a);
	}
	...
	/// \throws std::domain_error If the argument is outside the range [-1.0, 1.0].
	double Math::asin(const double a)
	{
		return std::asin(a);
	}
	...
	double Math::atan(const double a)
	{
		return std::atan(a);
	}
	...
	double Math::cbrt(const double a)
	{
		return std::cbrt(a);
	}
	...
	double Math::sqrt(const double a)
	{
		return std::sqrt(a);
	}
	...
	double Math::ceil(const double a)
	{
		return std::ceil(a);
	}
	...
	double Math::floor(const double a)
	{
		return std::floor(a);
	}

	/// \brief Returns the double value that is closest in value to the argument and is equal to a mathematical integer.
	/// \details If two double values that are mathematical integers are equally close to the value of the argument, the result is the integer value that is even.
	...
	double Math::rint(const double a)
	{
	...
	/// If the argument is halfway between two long values, the result is the long value that is closer to positive infinity.
	...
	long Math::round(const double a)
	{
	...
	double Math::exp(const double a)
	{
		return std::exp(a);
	}
	...
	double Math::log(const double a)
	{
		return std::log(a);
	}
	...
	double Math::log10(const double a)
	{
		return std::log10(a);
	}
	...
	double Math::sin(const double a)
	{
		return std::sin(a);
	}
	...
	double Math::cos(const double a)
	{
		return std::cos(a);
	}
	...
	double Math::toDegrees(const double radians)
	{
	...
	/// \details The ULP is the positive distance between the given double value and the next larger representable double value.
	/// \param[in] d The double value whose ULP is to be determined.
	/// \return The ULP of the argument.
	double Math::ulp(const double d)
	{
		return std::nextafter(d, std::numeric_limits<double>::infinity()) - d;
	}

	/// \brief Returns the unit in the last place (ULP) of a float value.
	/// \details The ULP is the positive distance between the given float value and the next larger representable float value.
	/// \param[in] f The float value whose ULP is to be determined.
	/// \return The ULP of the argument.
	float Math::ulp(const float f)
	{
		return std::nextafterf(f, std::numeric_limits<float>::infinity()) - f;
	}

	/// \brief Returns the signum function of a double value.
	/// \details The signum function returns -1 if the argument is less than zero, 0 if the argument is zero, and 1 if the argument is greater than zero.
	/// \param[in] d The double value whose signum is to be determined.
	/// \return The signum of the argument.
	double Math::signum(const double d)
	{
		return (d > 0) - (d < 0);
	}

	/// \brief Returns the signum function of a float value.
	/// \details The signum function returns -1.0f if the argument is less than zero, 0.0f if the argument is zero, and 1.0f if the argument is greater than zero.
	/// \param[in] f The float value whose signum is to be determined.
	/// \return The signum of the argument.
	float Math::signum(const float f)
	{
	...
	double Math::hypot(const double x, const double y)
	{
	...
	double Math::copySign(const double magnitude, const double sign)
	{
