...
double Math::abs(const double a) { return std::fabs(a); }
...
/// \return The absolute value of the argument.
float Math::abs(const float a) { return std::fabs(a); }
...
/// \return The absolute value of the argument.
int Math::abs(const int a) { return std::abs(a); }
...
/// \return The absolute value of the argument.
long Math::abs(const long a) { return std::labs(a); }
...
double Math::acos(const double a) { return std::acos(a); }
...
/// \throws std::domain_error If the argument is outside the range [-1.0, 1.0].
double Math::asin(const double a) { return std::asin(a); }
...
double Math::atan(const double a) { return std::atan(a); }
...
double Math::atan2(const double y, const double x) { return std::atan2(y, x); }
...
double Math::cbrt(const double a) { return std::cbrt(a); }
...
double Math::sqrt(const double a) { return std::sqrt(a); }
...
double Math::ceil(const double a) { return std::ceil(a); }
...
double Math::floor(const double a) { return std::floor(a); }
...
double Math::rint(const double a) { return std::nearbyint(a); }
...
long Math::round(const double a) { return static_cast<long>(std::round(a)); }
...
int Math::round(const float a) { return static_cast<int>(std::round(a)); }
...
double Math::exp(const double a) { return std::exp(a); }
...
double Math::log(const double a) { return std::log(a); }
...
double Math::log10(const double a) { return std::log10(a); }
...
double Math::pow(const double a, const double b) { return std::pow(a, b); }
...
double Math::sin(const double a) { return std::sin(a); }
...
double Math::cos(const double a) { return std::cos(a); }
...
double Math::tan(const double a) { return std::tan(a); }
...
double Math::sinh(const double x) { return std::sinh(x); }
...
double Math::cosh(const double x) { return std::cosh(x); }
...
double Math::tanh(const double x) { return std::tanh(x); }
...
double Math::toDegrees(const double radians) { return radians * 180.0 / std::numbers::pi; }
...
double Math::toRadians(const double degrees) { return degrees * std::numbers::pi / 180.0; }
...
double Math::ulp(const double d) { return std::nextafter(d, std::numeric_limits<double>::infinity()) - d; }
...
/// \return The ULP of the argument.
float Math::ulp(const float f) { return std::nextafterf(f, std::numeric_limits<float>::infinity()) - f; }
...
double Math::signum(const double d) { return (d > 0) - (d < 0); }
...
/// \return The signum of the argument.
float Math::signum(const float f) { return static_cast<float>((f > 0) - (f < 0)); }
...
double Math::hypot(const double x, const double y) { return std::hypot(x, y); }
...
double Math::copySign(const double magnitude, const double sign) { return std::copysign(magnitude, sign); }
} // namespace common::math
