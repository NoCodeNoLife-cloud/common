...
template <typename ReturnType, typename... Args> class TaskWrapper final : public interface::Runnable<ReturnType, Args...>
{
public:
...
template <typename ReturnType, typename... Args> TaskWrapper<ReturnType, Args...>::TaskWrapper(std::function<ReturnType(Args...)> func, std::function<void()> then) : mainFunc_(std::move(func)),
...
template <typename ReturnType, typename... Args> auto TaskWrapper<ReturnType, Args...>::SetMainFunction(std::function<ReturnType(Args...)> func) -> void { mainFunc_ = std::move(func); }

/// \brief Set the then function to be called after the main function is finished.
/// \param then The then function to be called.
template <typename ReturnType, typename... Args> auto TaskWrapper<ReturnType, Args...>::SetThenFunction(std::function<void()> then) -> void { thenFunc_ = std::move(then); }
} // namespace common::thread
